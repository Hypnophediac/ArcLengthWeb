...
class BendingCalculator:
    ...
    def draw_bending(self, inner_radius, neutral_radius, outer_radius,
        ...
        # A hajlítási szög a teljes szög osztva a hajlítások számával (most már pontosan 4 hajlítás)
        bend_angle = total_angle / segment_count
        # A kiegészítő szög: 180° - hajlítási szög
        complementary_angle = 180 - bend_angle

        # Lista a húrok hosszának tárolásához
        chord_lengths = []
        ...
        for i in range(segment_count):  # Pontosan annyi pont, ahány hajlítás
            ...
            if i < segment_count - 1:  # Egy húrral kevesebb mint hajlítási pont
                ...
                # Húr hosszának számítása (valós méretben, nem skálázva)
                dx = selected_radius * (math.cos(next_angle) - math.cos(angle))
                dy = selected_radius * (math.sin(angle) - math.sin(next_angle))
                chord_length = math.sqrt(dx*dx + dy*dy)
                
                # Húr rajzolása
                self.canvas.create_line(x, y, next_x, next_y,
                    fill='black', width=1, dash=(2, 2)
                )
                
                # Húr hosszának megjelenítése
                mid_x = (x + next_x) / 2
                mid_y = (y + next_y) / 2
                self.canvas.create_text(
                    mid_x, mid_y,
